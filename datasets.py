...
class agnews_custom(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
        self.root = root
        self.dataidxs = dataidxs
        self.train = train
        ...
        self.data, self.target = self.__build_truncated_dataset__()
    def __build_truncated_dataset__(self):
        X_train, y_train, X_test, y_test = utils.load_agnews_data(".")
        ...
        X_train, X_train_lens = list(zip(*data))
        X_train = torch.Tensor(X_train).type(torch.int64)
        X_train_lens = torch.Tensor(X_train_lens).type(torch.int64)
        y_train = torch.Tensor(target).type(torch.int64)
        data = [(x, lens) for x, lens in zip(X_train, X_train_lens)]
        target = y_train
        return data, target
    def __getitem__(self, index):
        X, target = self.data[index], self.target[index]
        return X, target
    def __len__(self):
        return len(self.data)

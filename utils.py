...
import datetime 
from model import *
from datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, ImageFolder_custom, SVHN_custom, FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData, agnews_custom
...
def load_agnews_data(datadir):
    max_len = 200
    trainset, testset = torchtext.datasets.AG_NEWS(root="./data/rawdata")
    trainlabel, traintext = list(zip(*trainset))
    testlabel, testtext = list(zip(*testset))
    # print(traintext)

    tokenizer = get_tokenizer('basic_english')
    vocab = build_vocab_from_iterator(map(tokenizer, iter(traintext+testtext)), specials=["<unk>"])
    ...
    test_label_list, test_text_list = text_transform(traintext, trainlabel, max_len)
    ...
    X_train = np.array(train_text_list, dtype=object)
    y_train = np.array(train_label_list)
    X_test = np.array(test_text_list, dtype=object)
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    #    np.save("data/generated/y_test.npy",y_test)
        ...
        X_train, y_train, X_test, y_test = load_agnews_data(datadir)
    ...
    elif partition > "noniid-#label000" and partition <= "noniid-#label999":
...
def compute_accuracy(model, dataloader, get_confusion_matrix=False, moon_model=False, device="cpu", special=False, mode=0, dataset="cifar10"):
        ...
        for tmp in dataloader:
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                if moon_model:
                    _, _, out = model(x)
                    if(special):
                        out = model.forward_method_moon(x)
                else:
                    if(special):
                        out = model.forward_method(x, mode)
                    else:
                        out = model(x)
    ...
    if get_confusion_matrix:
        return correct/float(total), conf_matrix, avg_time
    
    return correct/float(total), avg_time
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0, aux=0):
    if dataset in ('mnist', 'femnist', 'fmnist', 'cifar10', 'svhn', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY', 'cifar100', 'tinyimagenet', 'caltech','people','face','agnews'):
        if dataset == 'mnist':
        ...
        elif dataset == 'tinyimagenet' or dataset == 'face':
        ...
        elif dataset == 'caltech' or dataset == 'people':
            dl_obj = ImageFolder_custom
            transform_train = transforms.Compose([
                # transforms.ToPILImage(),
                transforms.RandomCrop(32, padding=4),
                ...
                transforms.RandomRotation(15),
                transforms.ToTensor(),
                # AddGaussianNoise(0., 1, net_id, total)
                # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
            ])
            transform_test = transforms.Compose([
                # transforms.ToPILImage(),
        ...
        if dataset == "tinyimagenet":
            ...
            train_ds = dl_obj(datadir+'./office_caltech_10/amazon', dataidxs=dataidxs, transform=transform_train)
            test_ds = dl_obj(datadir+'./office_caltech_10/amazon', transform=transform_test)
        elif dataset == 'people':
            train_ds = dl_obj(datadir+'./people', dataidxs=dataidxs, transform=transform_train)
            test_ds = dl_obj(datadir+'./people', transform=transform_test)
        elif dataset == 'face':
            train_ds = dl_obj(datadir+'./wiki_crop', dataidxs=dataidxs, transform=transform_train)
            test_ds = dl_obj(datadir+'./wiki_crop', transform=transform_test)
        else:
...
def lr_scheduler(rounds, args):
    # learning rate scheduler for decaying
    if rounds != 0:
        args.lr *= args.lr_decay #0.99
